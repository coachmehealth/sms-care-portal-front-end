...
const MessageTemplateTable: React.FC<TableProps> = ({ title, data, columns, options }: TableProps) => {
    ...
    const handelDelete = (rowID: string) => {
        const reqData = {
            id: rowID,
        };
        setSortedData(sortedData.filter((tableRow) => 
            rowID.localeCompare(tableRow._id) != 0
        ));
                ...
                <tbody>
                    {sortedData.slice(page * perPage, (page + 1) * perPage).map((row: any, ind) => (
                        <BodyRow key={`${ind}`}>
                            ...
                            <TableCell><Button onClick={() => handelDelete(row._id)}>Delete</Button></TableCell>
                        </BodyRow>
                    ))}
                </tbody>
            </StyledTable>
